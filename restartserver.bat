...
cd R:\arma3
timeout 3
start "arma3" /min /high "arma3server.exe" "-profiles=R:\arma3\config" "-BEPath=R:\arma3\TADST\1\BattlEye" "-config=R:\arma3\config\config.cfg" "-cfg=R:\arma3\config\basic.cfg" -port=2302 -world=empty -autoinit -noSound -nologs -malloc=tbbmalloc -nosplash -cpuCount=4 -maxmem=3047 "-mod=@extDB;@life_server;@RLP;@asm"
timeout 6
