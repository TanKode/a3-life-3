...
if(side _killer == west && playerSide != west) then {
	...
	if(!life_use_atm && {life_RLP_handgeld > 0}) then {
		[format[localize "STR_Cop_RobberDead",[life_RLP_handgeld] call life_fnc_numberText],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
		life_RLP_handgeld = 0;
	};
};
