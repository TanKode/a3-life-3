...
if((_price * _amount) > life_RLP_handgeld && {!isNil "_hideout" && {!isNil {grpPlayer getVariable "gang_bank"}} && {(grpPlayer getVariable "gang_bank") <= _price * _amount}}) exitWith {hint localize "STR_NOTF_NotEnoughMoney"};
	...
	if(!isNil "_hideout" && {!isNil {grpPlayer getVariable "gang_bank"}} && {(grpPlayer getVariable "gang_bank") >= _price}) then {
		_action = [
			format[(localize "STR_Shop_Virt_Gang_FundsMSG")+ "<br/><br/>" +(localize "STR_Shop_Virt_Gang_Funds")+ " <t color='#8cff9b'>$%1</t><br/>" +(localize "STR_Shop_Virt_YourFunds")+ " <t color='#8cff9b'>$%2</t>",
				...
				[life_RLP_handgeld] call life_fnc_numberText
			],
		...
		if(_action) then {
			...
			if((_price * _amount) > life_RLP_handgeld) exitWith {[false,_type,_amount] call life_fnc_handleInv; hint localize "STR_NOTF_NotEnoughMoney";};
			...
			__SUB__(life_RLP_handgeld,_price * _amount);
		};
	} else {
		if((_price * _amount) > life_RLP_handgeld) exitWith {hint localize "STR_NOTF_NotEnoughMoney"; [false,_type,_amount] call life_fnc_handleInv;};
		_icon = format["<img image='%1'/>",[([_type,0] call life_fnc_varHandle)] call life_fnc_itemIcon];
		hint parseText format[localize "STR_Shop_Virt_BoughtItem",_amount,_icon,_name,[(_price * _amount)] call life_fnc_numberText];
		__SUB__(life_RLP_handgeld,(_price * _amount));
	};
