	...
	switch(_item) do
	{
		...
		case "life_RLP_handgeld":
		{
			if(life_RLP_handgeld > 0) then
			{
				...
				_obj setPos _pos;
				[[player, player, _value, 2, life_RLP_atmgeld, life_RLP_handgeld],"TON_fnc_handleDBLog",false] spawn life_fnc_MP;
...
} foreach (life_inv_items + ["life_RLP_handgeld"]);