...
D41_BlitzerListe = ["SpeedRadar4"];
_Blitzer = nearestObjects [player,D41_BlitzerListe, 5];
_Blitzer = _Blitzer select 0;
if(_Blitzer getVariable["RLP_blitzer",false]) exitWith {hint format ["Es wurde bereits eine Messung gestartet", _Blitzer]};
_Blitzer setVariable ["RLP_blitzer", true, true];
...
while{true}do
{
	...
	if(!(_Blitzer getVariable "RLP_blitzer")) exitWith {hint "Messung abgebrochen"};
	sleep 0.5;
	...
	if(D41_KarreCheck) then
		{
			if(((speed D41_Karre)-5) > _MaxGeschw) then
			{
				...
				[[_MaxGeschw],"life_fnc_RLP_BlitzerFlash",D41_Karre,false] call life_fnc_MP;
				sleep 3;
			};
		};
};