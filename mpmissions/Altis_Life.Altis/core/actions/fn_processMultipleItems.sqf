...
if (!_hasLicense) then {
	...
	if ((floor (life_RLP_handgeld / _processingCost)) < _productionAmount) then {
		_productionAmount = floor (life_RLP_handgeld / _processingCost);
	};
};
...
if (_inputItemsDeleted) then {
	if (!_hasLicense) then {
		life_RLP_handgeld = life_RLP_handgeld - (_processingCost * _productionAmount);
	};
