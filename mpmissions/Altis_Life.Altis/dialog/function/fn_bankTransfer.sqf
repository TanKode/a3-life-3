...
if(_val > life_RLP_atmgeld) exitWith {hint localize "STR_ATM_NotEnough"};
_tax = [_val] call life_fnc_taxRate;
if((_val + _tax) > life_RLP_atmgeld) exitWith {hint format[localize "STR_ATM_SentMoneyFail",_val,_tax]};

life_RLP_atmgeld = life_RLP_atmgeld - (_val + _tax);

[[_val,profileName],"TON_fnc_clientWireTransfer",_unit,false] spawn life_fnc_MP;
[[player, _unit, _val, 0, life_RLP_atmgeld, life_RLP_handgeld],"TON_fnc_handleDBLog",false] spawn life_fnc_MP;
